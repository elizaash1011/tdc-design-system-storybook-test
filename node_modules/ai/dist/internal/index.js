...
async function prepareTools({
  ...
  for (const [name12, tool2] of orderToolEntries({ tools, toolOrder })) {
...
function createToolSearchState({
  ...
  const searchTools = Object.entries(tools != null ? tools : {}).filter(
    ([, tool2]) => tool2.deferLoading || isToolSearch(tool2)
  );
  if (searchTools.length === 0) {
    return (activeTools) => activeTools;
  }
  const discovered = /* @__PURE__ */ new Set();
  const getCallers = (name12) => {
    var _a12;
    return (_a12 = getOwn(toolCallers, name12)) != null ? _a12 : [DIRECT_TOOL_CALL];
  };
  for (const [name12, tool2] of searchTools) {
    const callers = getCallers(name12);
    if (callers.some((name13) => {
      if (name13 === DIRECT_TOOL_CALL) {
        return false;
      }
      const caller = experimental_getToolCaller2(tools == null ? void 0 : tools[name13]);
      return (caller == null ? void 0 : caller.type) !== "local" || caller.prepareModelMessage == null;
    }) || isToolSearch(tool2) && tool2.deferLoading) {
      throw new InvalidArgumentError({
        parameter: "tools",
        value: name12,
        message: `tool "${name12}" must be callable directly or through code mode with toolDiscovery: 'conversation'. The search tool itself must not defer loading.`
      });
    }
  }
  return (activeTools, { toolsContext = {}, experimental_sandbox } = {}) => {
    ...
    return Object.fromEntries(
      entries.filter(([name12, tool2]) => !tool2.deferLoading || discovered.has(name12)).map(([searchName, tool2]) => {
        if (!isToolSearch(tool2)) {
          return [searchName, tool2];
        }
        const callers = getCallers(searchName).filter(
          (name12) => name12 === DIRECT_TOOL_CALL || Object.hasOwn(activeTools, name12)
        );
        const candidates = entries.filter(
          ([name12, candidate]) => candidate.deferLoading && !isToolSearch(candidate) && callers.some((caller) => getCallers(name12).includes(caller))
        );
        return [
          searchName,
          {
            ...
            execute: ({ query }) => {
              ...
              const matches = candidates.map(([name12, candidate]) => {
                const description = resolveToolDescription({
                  tool: candidate,
                  toolName: name12,
                  toolsContext,
                ...
                const nameTerms = tokenize(name12);
                ...
                return { name: name12, description, score };
              }).filter((match) => match.score > 0).sort((a, b) => b.score - a.score).slice(0, 5);
              for (const { name: name12 } of matches) {
                discovered.add(name12);
              }
              return {
                tools: matches.map(({ name: name12, description }) => ({
                  name: name12,
                  ...description == null ? {} : { description }
                }))
              };
...
function tokenize(text) {
  var _a12;
  return (_a12 = text.replace(/([a-z\d])([A-Z])/g, "$1 $2").toLowerCase().match(/[\p{L}\p{N}]+/gu)) != null ? _a12 : [];
}
  ...
  for (const approval of approvedToolApprovals) {
    ...
    if (isExecutableTool(tool2) && tool2.inputSchema != null) {
      const validation = await safeValidateTypes3({
        ...
        schema: asSchema3(tool2.inputSchema)
      });
