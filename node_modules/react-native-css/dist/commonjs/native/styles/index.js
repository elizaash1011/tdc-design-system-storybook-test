 ...
 */
function hasNonOverlappingProperties(left, right) {
  // Null safety check
  if (!left || !right) {
    return false;
  }
...
function flattenStyleArray(styleArray) {
  ...
  for (const item of styleArray) {
    ...
    if (item == null || typeof item !== "object" || Array.isArray(item) || Object.prototype.hasOwnProperty.call(item, _reactivity.VAR_SYMBOL)) {
      return styleArray;
    }
  }
 ...
 */
function filterCssVariables(value, depth = 0) {
  ...
  if (Array.isArray(value)) {
    ...
    for (const item of value) {
      ...
      if (filteredItem !== undefined && !(typeof filteredItem === "object" && filteredItem !== null && Object.prototype.hasOwnProperty.call(filteredItem, _reactivity.VAR_SYMBOL))) {
  ...
  if (typeof value === "object") {
    ...
    if (Object.prototype.hasOwnProperty.call(value, _reactivity.VAR_SYMBOL)) {
      return undefined;
    }
...
function getStyledProps(state, inline, adaptProps = props => props) {
  let result;
  ...
  for (const config of state.configs) {
    result = deepMergeConfig(config, nativeStyleMapping(config, adaptProps(styledProps?.normal)), inline, true);
    if (styledProps?.important) {
      result = deepMergeConfig(config, result, nativeStyleMapping(config, adaptProps(styledProps.important)));
    }
    if (result && config.target) {
      const path = Array.isArray(config.target) ? config.target : [config.target];
      let target = result;
      for (const key of path.slice(0, -1)) target = target?.[key];
      const key = path[path.length - 1];
      if (target && key && key in target) {
        computedTargets.push({
          path,
          value: target[key]
        });
      }
  ...
  if (result) {
    for (const {
      ...
      for (const key of path.slice(0, -1)) {
        ...
        target = target[key] = Array.isArray(existing) ? [...existing] : {
          ...existing
        };
      }
 ...
 */
function mergeDefinedProps(left, right) {
  const result = left ? {
    ...left
  } : {};
      ...
      if (right?.style === null || right?.style === undefined) {
        ...
        if (left?.style) {
          if (!filteredRightStyle) {
            ...
            const leftIsObject = typeof leftStyle === "object" && !Array.isArray(leftStyle);
            const rightIsObject = typeof filteredRightStyle === "object" && !Array.isArray(filteredRightStyle);
        ...
        if (left?.style) {
          ...
          } else if (typeof left.style === "object" && typeof right.style === "object") {
  ...
  if (Array.isArray(config.target)) {
    ...
    if (config.target.length === 1 && finalKey && rightIsInline) {
      ...
      if (rightValue === undefined || rightValue === null) {
        ...
        const leftIsObj = typeof leftValue === "object" && leftValue !== null && !Array.isArray(leftValue);
        const rightIsObj = typeof rightValue === "object" && rightValue !== null && !Array.isArray(rightValue);
