...
function useNativeCss(type, originalProps, configs = [{
  ...
  const [state, setState] = (0, _react.useState)(() => {
    ...
    const initialState = (0, _rules.updateRules)({
      ruleEffect,
    ...
    (0, _reactivity.cleanupEffect)(ruleEffect);
    return initialState;
  });
  (0, _react.useEffect)(() => {
    ...
    return () => {
      (0, _reactivity.cleanupEffect)(state.ruleEffect);
      (0, _reactivity.cleanupEffect)(state.styleEffect);
    };
  ...
  let props = (0, _index.getStyledProps)(state, originalProps, type === _reactNative.Image ? adaptImageProps : undefined);
  ...
  if (state.containers) {
    ...
    const containers = Object.fromEntries(Object.entries(state.containers).map(([name, container]) => [name, container.key === state.ruleEffectGetter ? {
      key: container.key,
      props: originalProps
...
function mappingToConfig(mapping) {
  return Object.entries(mapping).flatMap(([key, value]) => {
    if (value === true) {
      ...
      const nativeStyleMapping = mapping ? Object.fromEntries(Object.entries(mapping).map(([k, v]) => [k, v === true ? k : v])) : undefined;
...
function adaptImageProps(props) {
