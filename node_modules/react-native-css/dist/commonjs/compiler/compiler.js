...
function compile(code, options = {}) {
  ...
  let effectiveRem = options.inlineRem ?? undefined;
  ...
  const firstPassVisitor = {
    Rule(rule) {
      if (rule.type === "unknown" && ["cssInterop", "react-native"].includes(rule.value.name)) {
        throw new Error(`Unsupported @${rule.value.name} configuration. Use the compiler inlineVariables.exclude option to preserve variables, @media (prefers-color-scheme: dark) for dark mode, and React Native Appearance.setColorScheme() for manual selection.`);
      }
      ...
      if (rule.type === "style" && rule.value.selectors.some(selector => selector.some(part => part.type === "pseudo-class" && part.kind === "root") && selector.some(part => part.type === "class" || part.type === "attribute" && part.name === "class"))) {
        throw new Error("Class-qualified :root selectors are unsupported on native. Use @media (prefers-color-scheme: dark) for dark mode, and React Native Appearance.setColorScheme() for manual selection.");
      }
    }
  };
  if (effectiveRem !== false) {
    ...
    firstPassVisitor.Length = length => {
      ...
      return {
        unit: "px",
        value: (0, _declarations.round)(length.value * remMultiplier)
      };
    };
  }
    ...
    case "style":
      {
        ...
        mapping = {
          ...
          ...(0, _atRules.parsePropDeclarations)(declarationBlock?.declarations),
          ...(0, _atRules.parsePropDeclarations)(declarationBlock?.importantDeclarations)
        };
...
function extractMedia(mediaRule, builder, mapping) {
  ...
  for (const mediaQuery of mediaRule.query.mediaQueries) {
    ...
    if (!(0, _mediaQuery.parseMediaQuery)(mediaQuery, queryBuilder)) {
      continue;
    }
...
function extractContainer(containerRule, builder, mapping) {
  ...
  const condition = (0, _containerQuery.parseContainerCondition)(containerRule.condition, builder);
  if (!condition) {
...
function extractPropertyRule(propertyRule, builder) {
  const {
    initialValue,
    name
  } = propertyRule;
...
function parsePropertyInitialValue(component, builder) {
  switch (component.type) {
    case "length":
      return (0, _declarations.parseLength)(component.value, builder);
    case "number":
    case "integer":
      return (0, _declarations.round)(component.value);
    case "percentage":
      return `${(0, _declarations.round)(component.value * 100)}%`;
    case "color":
      return (0, _declarations.parseColor)(component.value, builder);
    case "angle":
      return (0, _declarations.parseAngle)(component.value, builder);
    case "length-percentage":
      return (0, _declarations.parseLength)(component.value, builder);
    case "token-list":
      return (0, _declarations.reduceParseUnparsed)(component.value, builder, "@property", false);
    case "custom-ident":
    case "literal":
      return component.value;
    case "repeated":
      {
        const results = component.value.components.map(c => parsePropertyInitialValue(c, builder)).filter(v => v !== undefined);
