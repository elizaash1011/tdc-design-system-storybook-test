...
function isMappingProperty(name) {
...
function parsePropDeclarations(declarations = []) {
  const mapping = {};
  for (const declaration of declarations) {
    if (declaration.property !== "custom" || !isMappingProperty(declaration.value.name)) {
      continue;
    }
    const {
      name,
    ...
    const target = value.filter(item => item.type === "token" && item.value.type !== "white-space");
    if (target.length === 0) continue;
    nativeMappingAtRuleBlock([{
      type: "token",
      value: {
        type: "ident",
        value: name === mappingProperty ? "*" : name.slice(mappingProperty.length + 1)
      }
    }, {
      type: "token",
      value: {
