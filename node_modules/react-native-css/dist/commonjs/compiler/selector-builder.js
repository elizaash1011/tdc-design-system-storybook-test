...
exports.getClassNameSelectors = getClassNameSelectors;
...
function getClassNameSelectors(selectors, options = {}) {
  if (!selectors.length) {
    return [];
  }
  return selectors.flatMap(selector => {
    ...
    if (isRootVariableSelector(selector)) {
      ...
      return parseComponents(selector.reverse(), options, root, root, specificity) ?? [];
    }
  });
}
function parseComponents([component, ...rest], options, root, ref, specificity) {
  ...
  switch (component.type) {
    ...
    case "type":
      {
        // div, span
        if (component.name === options.selectorPrefix || component.name === "html") {
    ...
    case "pseudo-element":
      {
        switch (component.kind) {
          ...
          case "placeholder":
            {
              specificity[_index.Specificity.PseudoElements] = (specificity[_index.Specificity.PseudoElements] ?? 0) + 1;
          ...
          case "hover":
            {
              ...
              specificity[_index.Specificity.PseudoClass] = (specificity[_index.Specificity.PseudoClass] ?? 0) + 1;
              return parseComponents(rest, options, root, ref, specificity);
            }
          case "active":
            {
          ...
          case "is":
            {
              ...
              const isWhereContainerQueries = component.selectors.flatMap(selector => {
              ...
              const parents = parseComponents(rest, options, root, ref, specificity);
              if (!parents) {
                return null;
              }
              ...
              return parents.flatMap(parent => {
                ...
                return isWhereContainerQueries.map(containerQuery => {
                  ...
                  if (m && m.length > 1) {
                    parent.mediaQuery = originalParent.mediaQuery ? [["&", [...originalParent.mediaQuery, m]]] : [m];
                  }
                  if (component.kind === "is") {
                    for (let i = 0; i < specificity.length; i++) {
                      const value = specificity[i];
                      if (value !== undefined) {
                        parent.specificity[i] = (parent.specificity[i] ?? 0) + value;
                      }
                    }
                  }
                  if (query.a || query.p || query.n !== undefined) {
                    parent.containerQuery = [...(originalParent.containerQuery ?? [])];
    ...
    case "attribute":
      {
        if (component.name === "dir") {
          ...
          const attributeQuery = component.name.startsWith("data-") ?
          ...
          ["a", component.name.startsWith("aria-") ? component.name : toRNProperty(component.name)];
          if (component.operation) {
            let operator;
            ...
            if (operator) {
              ...
              } else if (component.operation.caseSensitivity === "explicit-case-sensitive") {
                attributeQuery.push("s");
              }
            }
          }
          getAttributeQuery(ref).push(attributeQuery);
          specificity[_index.Specificity.ClassName] = (specificity[_index.Specificity.ClassName] ?? 0) + 1;
          return parseComponents(rest, options, root, ref, specificity);
        }
      }
    case "class":
      {
        if (component.name === options.selectorPrefix) {
          ...
          getAttributeQuery(ref).unshift(["a", "className", "~=", component.name]);
        } else {
...
function parseIsWhereComponents(type, selector, index = 0, queries) {
    ...
    case "universal":
      {
        ...
        if (!previous || previous.type !== "combinator" || previous.value !== "descendant") {
          return null;
        }
    ...
    case "attribute":
      {
        ...
        for (const query of queries) {
          if (type === "is") {
            query.specificity[_index.Specificity.ClassName] = (query.specificity[_index.Specificity.ClassName] ?? 0) + 1;
          }
    ...
    case "class":
      {
        ...
        for (const query of queries) {
          ...
          query.n = query.n ? `${query.n}.${component.name}` : `g:${component.name}`;
        }
        return parseIsWhereComponents(type, selector, index + 1, queries);
      }
  }
}
function isContainerQuery(value) {
  return !("type" in value);
}
function getPseudoClassesQuery(key) {
...
function isRootVariableSelector([first, ...rest]) {
  rest = rest.filter(item => item.type !== "nesting");
  return first && rest.length === 0 && first.type === "pseudo-class" && first.kind === "root";
}
function isUniversalSelector([first, second]) {
