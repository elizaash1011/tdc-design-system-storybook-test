...
import { parseLength } from "./declarations.js";
export function parseMediaQuery(query, builder) {
    ...
    case "feature":
      {
        ...
        return feature?.some(value => value === undefined) ? undefined : feature;
      }
    case "not":
      const mediaQuery = parseMediaQueryCondition(query.value, builder);
      return mediaQuery ? ["!", mediaQuery] : undefined;
    case "operation":
      const parsed = query.conditions.map(c => parseMediaQueryCondition(c, builder));
      if (query.operator === "and" && parsed.some(c => !c)) {
        return;
      }
      const mediaQueries = parsed.filter(v => !!v);
...
export function parseMediaFeatureValue(value, builder) {
  switch (value.type) {
    case "boolean":
    case "ident":
    case "integer":
...
export function parseMediaFeatureOperator(operator) {
  switch (operator) {
    case "equal":
      return "=";
    case "greater-than":
