...
export function testAttributes(queries, props, guards) {
  return queries.every(query => testAttribute(query, props, guards));
}
function testAttribute([type, prop, operator, testValue, caseSensitivity], props, guards) {
  let value = undefined;
  if (props) {
    if (type === "a") {
      value = props[prop];
  ...
  if (value === undefined || value === null || testValue === undefined) return false;
