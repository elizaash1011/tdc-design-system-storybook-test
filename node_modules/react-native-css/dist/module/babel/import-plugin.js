...
import { handleReactNativeWebIdentifierRequire, handleReactNativeWebImport, handleReactNativeWebInteropRequireDefault, handleReactNativeWebObjectPatternRequire } from "./react-native-web.js";
export default function ({
  types: t
}) {
  const processed = new WeakSet();
  const packageRoot = resolve(__dirname, basename(dirname(__dirname)) === "src" ? "../.." : "../../..");
    ...
    visitor: {
      ...
      VariableDeclaration(path, state) {
        ...
        if (t.isIdentifier(id) && t.isIdentifier(init.callee, {
          ...
          statements = handleReactNativeWebInteropRequireDefault(path, t, id.name, init, source, state.filename);
        }
        if (!statements) {
          return;
        }
