...
export function handleReactNativeWebImport(declaration, t, filename) {
  const {
    specifiers,
    source
  } = declaration;
  ...
  for (const specifier of specifiers) {
    if (t.isImportDefaultSpecifier(specifier)) {
      ...
      if (!name) {
        ...
        statements.push(t.importDeclaration([t.importSpecifier(specifier.local, t.identifier(name))], t.stringLiteral(`react-native-css/components/${name}`)));
      }
...
export function handleReactNativeWebIdentifierRequire(path, t, id, source, filename) {
...
export function handleReactNativeWebInteropRequireDefault(path, t, id, init, source, filename) {
  const parsed = parseReactNativeWebSource(source, filename);
  if (!parsed) return;
  const wrapped = t.cloneNode(init);
  wrapped.arguments = [t.callExpression(t.identifier("require"), [t.stringLiteral(parsed.source)])];
  return [t.variableDeclaration(path.node.kind, [t.variableDeclarator(t.identifier(id), wrapped)])];
}
export function handleReactNativeWebObjectPatternRequire(path, t, id, source, filename) {
  const parsedSource = parseReactNativeWebSource(source, filename);
  ...
  for (const identifier of id.properties) {
    if (t.isRestElement(identifier)) {
      ...
      return;
    } else if (identifier.computed || !(t.isIdentifier(identifier.value) && t.isIdentifier(identifier.key))) {
