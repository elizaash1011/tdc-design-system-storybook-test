import type { ClassicComponentClass, ComponentClass, ComponentProps, ComponentPropsWithRef, ComponentType, ForwardRefExoticComponent, FunctionComponent, ReactElement } from "react";
...
export interface Styled {
    <const C extends ReactComponent>(component: C, mapping?: undefined, options?: StyledOptions): ComponentType<ComponentPropsWithRef<C> & {
        className?: string;
    }>;
    <const C extends ReactComponent, const M extends StyledConfiguration<C>>(component: C, mapping: M & StyledConfiguration<C>, options?: StyledOptions): StyledComponent<C, M>;
}
type StyledComponent<C extends ReactComponent, M extends StyledConfiguration<C>> = ComponentType<ComponentPropsWithRef<C> & {
    [K in keyof M as K extends string ? M[K] extends undefined | false ? never : M[K] extends true | string | object ? K : never : never]?: string;
}>;
