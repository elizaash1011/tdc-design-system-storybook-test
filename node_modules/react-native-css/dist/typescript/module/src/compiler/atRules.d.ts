import type { Declaration, DeclarationBlock, ParsedComponent, Rule, TokenOrValue } from "lightningcss";
import type { StyleRuleMapping } from "./compiler.types.js";
import type { StylesheetBuilder } from "./stylesheet.js";
export interface PropAtRule {
    type: "unknown";
    value: {
        name: "prop";
        prelude: Extract<TokenOrValue, {
            type: "token";
        }>[];
        block: Extract<TokenOrValue, {
            type: "token";
        }>[] | null;
    };
}
/***********************************************
 * @react-native                               *
 ***********************************************/
export interface ReactNativeAtRule {
    type: "custom";
    value: {
        name: "react-native";
        prelude: null | Extract<ParsedComponent, {
            type: "repeated";
        }>;
        body: {
            type: "declaration-list";
            value: Pick<DeclarationBlock, "declarations">;
        };
    };
}
export declare function maybeMutateReactNativeOptions(rule: Rule | ReactNativeAtRule, _builder: StylesheetBuilder): void;
export declare function parsePropAtRule(rules?: (Rule | PropAtRule)[]): StyleRuleMapping;
export declare function isMappingProperty(name: string): boolean;
/** Declaration metadata survives the optimizer flattening nested at rules. */
export declare function parsePropDeclarations(declarations?: Declaration[]): StyleRuleMapping;
//# sourceMappingURL=atRules.d.ts.map